...
본인이 결정한 부분은 최종 알고리즘의 방향과 평가 기준이다. 단순한 직접 회귀보다 기하 해를 먼저 만들고 residual만 보정하는 구조를 선택했고, validation split과 seed별 반복 검증으로 과적합 가능성을 확인했다. Agent AI가 제안한 문장이나 코드 구조는 그대로 믿지 않고, 프로젝트 README 규칙과 실제 실행 결과를 기준으로 수정했다. 최종 제출 코드에서는 표준 환경에 포함된 numpy, scipy, scikit-learn만 사용했다.
